...
{
	long long unknown; //0x0000
	union
	{
		long long steamID64; //0x0008 - SteamID64
		struct
		{
			int xuid_low;
			int xuid_high;
	...
	unsigned int iSteamID; //0x00B8 - SteamID
...
class IVEngineClient
{
  public:
	...
	virtual void SpherePaintSurface(const model_t* model, const vec3& location, unsigned char chr, float fl1, float fl2) = 0;
	virtual bool HasPaintmap(void) = 0;
	virtual void EnablePaintmapRender() = 0;
	//virtual void TracePaintSurface( const model_t *model, const vec3& position, float radius, CUtlvec3<Color>& surfColors ) = 0;
	virtual void SphereTracePaintSurface(const model_t* model, const vec3& position, const vec3 &vec2, float radius, /*CUtlvec3<unsigned char, CUtlMemory<unsigned char, int>>*/ int& utilVecShit) = 0;
